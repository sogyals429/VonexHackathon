...
class WCFMmp_Frontend {
	...
	function wcfmmp_scripts() {
		...
		if( wcfmmp_is_stores_list_page() || wcfmmp_is_stores_map_page() ) {
			...
			wp_localize_script( 'wcfmmp_store_list_js', 'wcfmmp_store_list_options', array( 'is_geolocate' => apply_filters( 'wcfmmp_is_allow_store_list_by_user_location', true ), 'max_radius' => apply_filters( 'wcfmmp_radius_filter_max_distance', $max_radius_to_search ), 'default_lat' => $default_lat, 'default_lng' => $default_lng, 'is_poi' => apply_filters( 'wcfmmp_is_allow_map_poi', true ) ) );
		}
